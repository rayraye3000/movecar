...
function generateMapUrls(lat, lng) {
  ...
  return {
    ...
    baiduUrl: `https://api.map.baidu.com/marker?location=${bd.lat},${bd.lng}&title=位置&content=挪车位置&output=html`,
    tencentUrl: `https://apis.map.qq.com/uri/v1/marker?marker=coord:${gcj.lat},${gcj.lng};title:位置`,
    gcj: gcj,
    bd: bd
  };
}
...
async function handleNotify(request, url) {
  try {
    ...
    if (typeof TG_BOT_TOKEN !== 'undefined' && TG_BOT_TOKEN && typeof TG_CHAT_ID !== 'undefined' && TG_CHAT_ID) {
...
function getPhoneNumber() {
  try {
    return typeof PHONE_NUMBER !== 'undefined' ? PHONE_NUMBER : '';
  } catch (e) {
    return '';
  }
}
  ...
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; margin: 0; padding: 0; }
      ...
      .owner-card .map-btn:active { transform: scale(0.96); }
      
      .map-btn.amap { background: #1890ff; color: white; }
      .map-btn.apple { background: #1d1d1f; color: white; }
      .map-btn.baidu { background: #1a6cff; color: white; }
      .map-btn.tencent { background: #07c160; color: white; }
    ...
    <div class="container" id="successView">
      ...
      <div id="ownerFeedback" class="card owner-card hidden">
        ...
        <div id="ownerMapLinks" class="map-links" style="display:none">
          <a id="ownerAmapLink" href="#" class="map-btn amap">🗺️ 高德</a>
          <a id="ownerBaiduLink" href="#" class="map-btn baidu">🗺️ 百度</a>
          <a id="ownerTencentLink" href="#" class="map-btn tencent">🗺️ 腾讯</a>
          <a id="ownerAppleLink" href="#" class="map-btn apple">🍎 苹果</a>
        </div>
      </div>
      ...
      <div class="card action-card">
        ...
        ${phone ? `<a href="tel:${phone}" class="btn-phone"><span>📞</span><span>直接打电话</span></a>` : ''}
      </div>
    </div>
          ...
          try {
            ...
            if (data.status === 'confirmed') {
              ...
              if (data.ownerLocation) {
                const links = document.getElementById('ownerMapLinks');
                links.style.display = 'grid';
                if (data.ownerLocation.amapUrl) {
                  document.getElementById('ownerAmapLink').href = data.ownerLocation.amapUrl;
                }
                if (data.ownerLocation.baiduUrl) {
                  document.getElementById('ownerBaiduLink').href = data.ownerLocation.baiduUrl;
                }
                if (data.ownerLocation.tencentUrl) {
                  document.getElementById('ownerTencentLink').href = data.ownerLocation.tencentUrl;
                }
                if (data.ownerLocation.appleUrl) {
      ...
      function startCooldown(seconds) {
        ...
        cooldownTimer = setInterval(() => {
          ...
          if (cooldownSeconds <= 0) {
            ...
            display.textContent = \`⏳ 冷却中：\${minutes}分\${secs.toString().padStart(2, '0')}秒后可再次通知\`;
          }
        }, 1000);
      }
  ...
  <body>
    <div class="card">
      ...
      <div id="mapArea" class="map-section">
        <p>📍 对方位置</p>
        <div class="map-links">
          <a id="amapLink" href="#" class="map-btn amap">🗺️ 高德</a>
          <a id="baiduLink" href="#" class="map-btn baidu">🗺️ 百度</a>
          <a id="tencentLink" href="#" class="map-btn tencent">🗺️ 腾讯</a>
          <a id="appleLink" href="#" class="map-btn apple">🍎 苹果</a>
        </div>
      </div>
        ...
        try {
          ...
          if(res.ok) {
            ...
            if(data.amapUrl) {
              ...
              document.getElementById('baiduLink').href = data.baiduUrl || '#';
              document.getElementById('tencentLink').href = data.tencentUrl || '#';
